...
fn normalize_trailing_newlines(doc_code: &str, repo_code: String) -> (String, Option<Fix>) {
    ...
    let newlines_fix = Fix::TrailingNewline {
...
fn normalize(doc_code: &str, repo_code: &str) -> (String, Vec<Fix>) {
...
fn unused_section_name(markers: &Markers, block: &Block) -> String {
...
fn portion_plan(block: &Block, doc_code: &str, file_text: &str, markers: &Markers) -> Option<Vec<Fix>> {
...
fn whole_file_plan(doc_code: &str, file_text: &str) -> Result<Option<Vec<Fix>>> {
    ...
    content_plan(doc_code, &marked_text, vec![Fix::MarkFile], true, &[])
}

fn plan_against(block: &Block, doc_code: &str, candidate: &Candidate<'_>) -> Result<Option<Vec<Fix>>> {
    if let Some(marked_code) = candidate.marked {
        return content_plan(
            doc_code,
            &marked_code.content,
            vec![],
    ...
    if candidate.markers.sections.is_empty()
        && let Some(fixes) = whole_file_plan(doc_code, candidate.text).context("marking the whole file")?
    {
        return Ok(Some(fixes));
    }
    Ok(portion_plan(block, doc_code, candidate.text, candidate.markers))
}

/// The lightbulb for a candidate, if any. When shell prompts also stand in the
/// way, it adds `doc strip-line-prefix: "$ "` to the marker.
pub(crate) fn plan_for(block: &Block, candidate: &Candidate<'_>) -> Result<Option<Plan>> {
    ...
    let Ok(doc_code) = markers::apply_options(&block.content, &[prompt_option()]) else {
        return Ok(None);
    };
...
fn edit_section(
    ...
    let (before, marked_region, after) =
        split_around_range(text, section.from, section.to).context("locating the section")?;
...
fn mark_section(
    ...
    let split_lines = first_line.checked_sub(1).and_then(|lines_before_count| {
        let (before, from_first) = text_lines.split_at_checked(lines_before_count)?;
        let (marked, after) = from_first.split_at_checked(line_count)?;
        Some((before, from_first, marked, after))
    });
    let Some((before, from_first, marked, after)) = split_lines else {
        bail!("the {line_count} lines from line {first_line} aren't all in the file");
    };
    let indent: String = from_first
        .first()
        ...
        .unwrap_or_default();
    let start_marker_lines = iter::once(format!("{indent}# {}", markers::section_start(section_name))).chain(
        options
            .iter()
            .map(|option| format!("{indent}#   | {}", option.format())),
    );
    let end_marker_line = format!("{indent}# {}", markers::section_end(section_name));
    let to_owned_lines = |borrowed_lines: &[&str]| borrowed_lines.iter().map(ToString::to_string).collect::<Vec<_>>();
    Ok(to_owned_lines(before)
        .into_iter()
        .chain(start_marker_lines)
        .chain(to_owned_lines(marked))
        .chain(iter::once(end_marker_line))
        .chain(to_owned_lines(after))
        .collect::<Vec<_>>()
        .join("\n"))
}
...
fn apply_fix(text: &str, section: Option<&str>, fix: &Fix, doc_options: &[MarkerOption]) -> Result<String> {
    match fix {
        ...
        Fix::MarkSection {
            ...
            mark_section(text, name, *line, *lines, &all_options).with_context(|| format!("marking section \"{name}\""))
        }
        ...
        Fix::TrailingNewline { wanted_newlines, .. } => edit_region(text, section, |region| {
...
pub(crate) fn apply(repo_root: &Path, file: &str, section: Option<&str>, plan: &Plan) -> Result<()> {
